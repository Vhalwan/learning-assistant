...
def render(st: Any, stem: str, text: str, llm, hist_key: str):
    ...
    for idx, q in enumerate(quiz_items, start=1):
        ...
        # Expander title: short preview of question (keeps same expanded state behavior)
        title_preview = q_text[:120].replace("\n", " ")
        expander_title = f"Q{idx} ({short_id}...){status_icon}: {title_preview}"

        # Use expander to hold the full question & controls (preserve expand state)
        with st.expander(expander_title, expanded=st.session_state[exp_key]):
            ...
            # Action buttons row: Check answer | Start SRS | (optionally) Toggle explanation
            ...
            with btn_col_left:
                ...
                if st.button("Check answer", key=check_key, disabled=check_disabled):
                    ...
                    # open the expander to show results & explanation after checking
                ...
                if is_correct:
                    ...
                    if explanation_text:
                        ...
                        with st.expander("View explanation", expanded=True):
                            # long explanations may be long: put into a text area for scrolling
                    ...
                    # show explanation if available, in a collapsible box
                    if explanation_text:
                        with st.expander("View explanation", expanded=True):
                            if len(explanation_text) > 300:
                                st.text_area("", value=explanation_text, height=160, disabled=True)
